...
def set_wallpaper(image_file):
    ...
    # the fourth zero specifies the positioning of the wallpaper, in this case 0 means the wallpaper should be centered.

def get_image_file(image_directory):
    # Function retrieves random img file from folder
...
def rotate_background(image_directory):
    ...
    if image_file:
        ...
        print(f"Desktop background set to: {image_file}")
    else:
...
def main():
    ...
    schedule.every(1).hours.do(rotate_background, image_directory)
